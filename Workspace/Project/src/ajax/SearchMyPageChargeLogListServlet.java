...
import ajax.response.AjaxSearchChargePointLogListResponse;
...
@WebServlet("/SearchMyPageChargeLogListServlet")
public class SearchMyPageChargeLogListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		AjaxSearchChargePointLogListResponse ajaxResponse = new AjaxSearchChargePointLogListResponse();
		...
		DBResult dbResult = LogILDBManager.getInstance().listChargePointLogs(user_uid, pageNumber, new PageOption(pageOption, pageLimit));
		dbResult.println();
		
		if (dbResult.getStatus() == DBResult.SUCCESS) {
			
			Tuple<Integer, Page<ChargePointLogBean>> data = ((DBResultWithData<Tuple<Integer, Page<ChargePointLogBean>>>)dbResult).getData();
