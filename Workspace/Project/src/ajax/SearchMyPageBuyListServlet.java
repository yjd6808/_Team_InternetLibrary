...
import ajax.response.AjaxSearchBuyListResponse;
...
@WebServlet("/SearchMyPageBuyListServlet")
public class SearchMyPageBuyListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		AjaxSearchBuyListResponse ajaxResponse = new AjaxSearchBuyListResponse();
		...
		DBResult dbResult = MyBookILDBManager.getInstance().listBuyBooks(user_uid, pageNumber, new PageOption(pageOption, pageLimit));
		dbResult.println();
		
		if (dbResult.getStatus() == DBResult.SUCCESS) {
			
			Tuple<Integer, Page<BuyBookShortcutBean>> data = ((DBResultWithData<Tuple<Integer, Page<BuyBookShortcutBean>>>)dbResult).getData();
			ajaxResponse.setTotalBuyBookCount(data.getItem1());
