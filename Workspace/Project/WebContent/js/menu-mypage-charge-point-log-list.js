...
async function search(pageNumber, pageOption) {
	...
	axios.get('/SearchMyPageChargeLogListServlet?' +
	...
	.then((response) => {
		...
		alert('포인트 충전내역 로딩 중 오류가 발생하였습니다\n' + err);
	});
...
function updateTableBody(page) {
	...
	if (page === null || page.datas.length == 0) {
		html += '<tr>';
		html += '<td colspan="6" class="p-3">충전 내역이 없습니다.</td>';
		html += '</tr>'
	...
	/**
		...
		<td>내역</td>
		<td style="width: 15%;">충전 날짜</td>
		<td style="width: 15%;">충전 포인트</td>
	...
	for (let i = 0; i < page.datas.length; i++) {
		...
		html += `<td>포인트 충전</td>`;
		html += `<td>${new Date(page.datas[i].chargeDate).customFormat("#YYYY#-#DD#-#MM#")}<br>${new Date(page.datas[i].chargeDate).customFormat("#ampm# #h#:#mm#:#ss#")}</td>`;
		html += `<td>${numberWithCommas(page.datas[i].chargePoint)}P</td>`;
		html += `<td>${numberWithCommas(page.datas[i].price)}원</td>`;
		html += '</tr>'
	}
