...
async function search(pageNumber, pageOption) {
	...
	axios.get('/SearchMyPageBorrowListServlet?' +
	...
	.then((response) => {
		...
		alert('도서 대여 게시글 로딩 중 오류가 발생하였습니다\n' + err);
	});
...
function updateTableBody(page) {
	...
	if (page === null || page.datas.length == 0) {
		html += '<tr>';
		html += '<td colspan="6" class="p-3">대여중인 도서가 없습니다.</td>';
		html += '</tr>'
	...
	for (let i = 0; i < page.datas.length; i++) {
		...
		html += `<td>${page.datas[i].startDate}</td>`;
		html += `<td>${page.datas[i].endDate}</td>`;
        html += `<td>${page.datas[i].leftTime}</td>`;
		html += '</tr>'
	}
